...
import { AlertCircle, Camera, ChevronDown, FileText, Loader2 } from 'lucide-react';
...
export const AssistantTextMessage = ({ text }: { text: string }) => {
  return (
    <div className="flex gap-2 mb-4 items-center">
      <div className="mr-auto px-3 pt-3 pb-1 rounded-md bg-accent">
...
export const AttachedFilesMessage = ({
  files,
}: {
  files: ProcessedFile[];
}) => {
  return (
    <div className="flex gap-2 flex-wrap mt-1 ml-auto mr-4">
      {files.map((file, i) => (
        <div
          key={i}
          className="border rounded-lg p-1.5 text-xs flex items-center gap-1.5 bg-secondary"
        >
          {file.type === 'image' ? (
            <img
