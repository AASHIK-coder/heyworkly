...
const LocalOperator = () => {
  ...
  useEffect(() => {
    ...
    // Only sync messages for the active session — skip if another session
    // was activated (e.g. during new-chat transition where currentSessionId
    // updates before the router state does)
    if (
      currentSessionId &&
      state.sessionId &&
  ...
  }, [pendingAction, onNewChat, onBack]);
        ...
        <div ref={containerRef}>
          ...
          {chatMessages?.map((message, idx) => {
            if (message?.from === 'human') {
              ...
              return (
                <div key={`message-${idx}`}>
                  <HumanTextMessage text={message?.value} />
                  {message.attachments && message.attachments.length > 0 && (
                    <AttachedFilesMessage files={message.attachments} />
                  )}
                </div>
              );
            }
    ...
    <div className="flex flex-col w-full h-full">
      ...
      <div className="px-5 pb-5 flex flex-1 gap-5">
        ...
        <Card className="flex-1 basis-3/5 p-3 h-[calc(100vh-76px)]">
          {state.operator === Operator.LocalBrowser ? (
            <WorkspacePanel
