...
// Every externalized package with runtime dependencies needs its full
// transitive dependency tree copied into the packaged node_modules.
// flora-colossus walks each package recursively, so we only list the
// top-level externalized packages that have deps (jose and pdfjs-dist
// have zero runtime deps so they're excluded).
...
const config: ForgeConfig = {
  ...
  plugins: [
    ...
    // Fuses modify the Electron binary at package time. Several fuses
    // (integrity validation, OnlyLoadAppFromAsar) cause "damaged" errors
    // when the app is not properly code-signed. Only enable fuses when
    // we have signing credentials.
    // https://github.com/microsoft/playwright/issues/28669#issuecomment-2268380066
    ...(process.env.CI === 'e2e' || !enableOsxSign
      ? []
      : [
          new FusesPlugin({
