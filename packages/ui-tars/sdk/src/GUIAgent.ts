...
> {
  ...
  async run(
    ...
    // Build initial conversations: instruction + any attached images
    const initialConversations: GUIAgentData['conversations'] = [
      {
        from: 'human',
        value: instruction,
        timing: {
    ...
    // Add user-uploaded images as IMAGE_PLACEHOLDER conversations
    if (initialImages && initialImages.length > 0) {
      for (const imgBase64 of initialImages) {
      ...
      logger.info(
        `[GUIAgent] Added ${initialImages.length} user-uploaded images to initial context`,
      );
    }
